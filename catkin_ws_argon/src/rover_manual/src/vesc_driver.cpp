...
#include <rover_manual/vesc_state_stamped.h>
  ...
  driver_mode_(MODE_INITIALIZING), fw_version_major_(-1), fw_version_minor_(-1)
{
  ...
  state_pub_ = nh.advertise<rover_manual::vesc_state_stamped>("sensors/core", 10);
...
void VescDriver::vescPacketCallback(const boost::shared_ptr<VescPacket const>& packet)
{
  if (packet->name() == "Values") {
    ...
    rover_manual::vesc_state_stamped::Ptr state_msg(new rover_manual::vesc_state_stamped);
